...
import { getElementInfo, shouldExcludeElement, getElementBounds } from './element-picker-utils'
import type { ElementInfo, ElementPickerProps, HighlightPosition, TooltipPosition } from './types'
...
export function ElementPicker({
  ...
  onSelect,
  onCancel,
  instructionText = 'Select an element',
  ...
  const updateFocusableElements = useCallback(() => {
    ...
    const elements = Array.from(document.querySelectorAll(selector)).filter(
      (el) =>
        !shouldExcludeElement(el, excludeAttribute) &&
        !el.closest(`[${excludeAttribute}-overlay]`)
    )
  ...
  const handleMouseMove = useCallback(
    (e: MouseEvent) => {
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        if (target && !shouldExcludeElement(target, excludeAttribute)) {
    ...
    [updateHighlight, excludeAttribute]
  )
  ...
  return createPortal(
    <div
      data-element-picker-overlay
      className="pointer-events-none fixed inset-0 z-[9999]"
      style={{ cursor: 'crosshair' }}
    >
      {/* Semi-transparent overlay - visual only */}
      <div data-element-picker className="absolute inset-0 bg-black/20" />
      ...
      >
        <div className="flex items-center gap-4 text-sm">
          <div className="flex items-center gap-2 text-gray-700 dark:text-gray-200">
            ...
            <span className="font-medium">{instructionText}</span>
          </div>
